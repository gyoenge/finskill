...
1. **공공 데이터 연동 현황** — 실연동이 3건 들어갔고 SH 등 일부만 시드로 남아 있습니다.

   | Skill | 데이터 | 비고 |
   |---|---|---|
   | LH 공고 실시간 조회 | ✅ 실시간 API | 공공데이터포털 15058530 |
   | 청년정책 실시간 조회 | ✅ 실시간 API | 온통청년 (자체 인증키) |
   | 장학금 탐색 | ✅ 실데이터(파일) | 2026.08.11 스냅샷 · 실시간 아님 |
   | SH 청년주택 검색 | ⚠️ 시드 | 전용 공개 API 를 찾지 못함 |
   | LH 청년주택 검색 | ⚠️ 시드 | 실시간 API 에 보증금·월세가 없어 병행 유지 |
   | 예·적금 비교 | ⚠️ 시드 | 금감원 finlife 키 미발급 |

   **장학금** — 원본이 실시간 API 가 아니라 주기 갱신 파일이라 과거 공고까지 들어 있습니다
   (1,859건 중 1,808건이 이미 마감). 생성 단계와 실행 시점에서 두 번 걸러 모집 중인 것만
   노출합니다. CSV 를 새로 받으면 다시 생성하면 됩니다:

   ```bash
   python3 scripts/build-scholarships.py data/한국장학재단_....csv
   ```

   **온통청년** — data.go.kr 의 해당 데이터셋은 API 유형이 `LINK` 라 data.go.kr 서비스키로는
   호출되지 않습니다. 온통청년 자체 인증키(`apiKeyNm`)가 필요하고, 구 엔드포인트
   (`/opi/youthPlcyList.do`)는 302 로 폐기되어 `/go/ythip/getPlcy` 를 씁니다.
   한글 파라미터는 반드시 URL 인코딩해야 하며, 안 하면 `400 Bad Request` 가 납니다.
